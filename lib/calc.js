...
// Zero-Tolerance Contractual Breach Rule:
// If ANY metric with Target = 0 (or binary0 lower-is-better) has computed value > 0 (even +1),
// then the overall month status MUST immediately become RED.
// Otherwise, when all Target = 0 metrics are 0 (or within SLA), status is determined by the majority base:
// e.g. green count highest -> green, red count highest -> red, amber highest -> amber.
function computeOverallRAG(monthMetrics) {
  if (!monthMetrics || typeof monthMetrics !== 'object') return null;

  let hasAny = false;
  let zeroTargetBreach = false;
  ...
  METRICS.forEach(metric => {
    ...
    const val = entry.computed;
    const hasComputed = val !== null && val !== undefined && Number.isFinite(Number(val));

    // Zero-tolerance Target = 0 check:
    // Any metric with explicit target === 0 or '0' or '0%', or binary0 with direction 'lower'
    let isZeroTarget = false;
    if (metric.target !== null && metric.target !== undefined && metric.target !== '' && metric.target !== '-') {
      const cleanT = String(metric.target).replace('%', '').trim();
      if ((cleanT === '0' || parseFloat(cleanT) === 0) && (metric.direction || '').toLowerCase() === 'lower') {
        isZeroTarget = true;
      }
    }
    if (metric.ragRule === 'binary0' && (metric.direction || '').toLowerCase() === 'lower') {
      isZeroTarget = true;
    }

    if (isZeroTarget && hasComputed && Number(val) > 0) {
      zeroTargetBreach = true;
    }

    if (entry.rag) {
      hasAny = true;
      const r = (entry.rag || '').toLowerCase();
      if (r === 'green') greenCount++;
      else if (r === 'amber') amberCount++;
      else if (r === 'red') redCount++;
    }
  });

  if (!hasAny) return null;

  // 1. Critical Zero-Tolerance Rule: If any target=0 metric increases > 0, overall is RED!
  if (zeroTargetBreach) {
    return 'red';
  }

  // 2. Majority Base Rule (when all target=0 metrics are 0 or within SLA):
  if (greenCount > redCount && greenCount >= amberCount) {
    return 'green';
  }
  if (redCount > greenCount && redCount >= amberCount) {
    return 'red';
  }
  if (amberCount > greenCount && amberCount > redCount) {
    return 'amber';
  }
  if (redCount === greenCount && redCount > 0) {
    return 'red'; // conservative tie-breaker
  }
