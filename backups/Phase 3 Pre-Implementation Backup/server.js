  ...
  try {
    ...
    if (pathname.startsWith('/api/users/') && req.method === 'PUT') {
      ...
      if (body.role) {
        ...
        if (user.role === 'superadmin' && body.role !== 'superadmin') {
          const superadmins = usersDB.users.filter(u => u.role === 'superadmin');
          if (superadmins.length <= 1) {
            return sendJSON(res, 400, { error: 'Cannot demote the last Super Admin' });
          }
        }
        user.role = body.role;
      }
    ...
    if (pathname.startsWith('/api/users/') && req.method === 'DELETE') {
      ...
      if (targetUser.role === 'superadmin') {
        const superadmins = usersDB.users.filter(u => u.role === 'superadmin');
        if (superadmins.length <= 1) {
          return sendJSON(res, 400, { error: 'Cannot delete the only Super Admin account' });
        }
      }
