...
function renderMetricCard(metric) {
  ...
  // Phase 1: All roles have full UI access. Permission-based UI locking is Phase 2.
  ...
  if (state.activeHistoryCard === metric.id && historyList.length > 0) {
    ...
    historyList.forEach((h, idx) => {
      ...
      // Phase 1: All roles see all history controls. Phase 2 will restrict by permission.
      itemsHtml += `
...
// Phase 1: All roles get full input access (no readonly/disabled). Phase 2 will restrict by permission.
...
function renderExecSummary() {
  ...
  // Phase 1: All roles can edit the executive summary. Phase 2 will add permission-based restrictions.
  ...
  narrativeWrap.innerHTML = `
    <div class="narrative-field">
      ...
      <textarea id="n-topRisks" placeholder="Key risks carried into next month...">${narrative.topRisks || ''}</textarea>
    </div>
    <div class="narrative-field">
      <label>2. Key Improvements Delivered This Month</label>
      <textarea id="n-improvements" placeholder="Improvements delivered this month...">${narrative.improvements || ''}</textarea>
    </div>
    <div class="narrative-field">
      <label>3. Planned Actions &amp; Objectives — Next Month</label>
      <textarea id="n-plannedActions" placeholder="Committed actions for the next reporting period...">${narrative.plannedActions || ''}</textarea>
    </div>
  ...
  frag.innerHTML = `
    <div class="users-header-row">
      <div class="users-header-info">
        ...
        <p>Manage cybersecurity console operators, provision accounts, and assign role clearances.</p>
      </div>
      ...
      tr.innerHTML = `
        ...
        <td style="text-align:right;">
          <div class="user-actions-group">
            <button type="button" class="btn-user-pwd" data-user-id="${u.id}" data-username="${escapeHTML(u.username)}" title="Reset user password">
            ...
            ${!isSelf ? `
              <button type="button" class="btn-user-delete" data-user-id="${u.id}" data-username="${escapeHTML(u.username)}" title="Delete operator account">
      ...
      if (pwdBtn) {
        pwdBtn.addEventListener('click', async () => {
          ...
          try {
            ...
            showToast('Password reset for @' + u.username, 'green');
          } catch (err) {
      ...
      if (delBtn) {
        delBtn.addEventListener('click', async () => {
          ...
          try {
            ...
            showToast('Operator @' + u.username + ' removed', 'green');
